...
class JongmanControllerInstance extends RFControllerForm
{
     ...
     * @param null $key
     * @param null $urlVar
     * @internal we use reference_number instead of id, and its type is string
     * @return bool
     *
     * @since 3.1
     */
	public function edit($key=null, $urlVar='reference_number')
	{
        ...
        $refNumber = count($cid) ? $cid[0] : $this->input->getString($urlVar);
        $checkin = property_exists($table, 'checked_out');

        $table->load(['reference_number' => $refNumber]);
        $recordId = $table->id;
        ...
        else
        {
            ...
            $this->setRedirect(
                JRoute::_(
                    ...
                    . $this->getRedirectToItemAppend($refNumber, 'reference_number'), false
                )
            );
	...
	public function allowEdit($data = array(), $key = 'id') 
	{
		...
		if (isset($data) && ($data[$key] > 0)) {
			...
			if ($actions->get('core.admin') || $actions->get('core.edit')) {
				return true;
